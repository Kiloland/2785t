...
void ScalerDpTxMstDownReplyProc(void)
{
    ...
    if((GET_DP_TX_DOWN_REPLY_READY() == _TRUE) && (GET_DP_TX_RD_BUFFER_STATUS() != _DP_TX_READ_BUF_UP_REQUEST_IN_USE) && (GET_DP_TX_DOWN_REPLY_READ_DISABLE() == _FALSE))
    {
        ...
        for(ucRetry = 0; ucRetry < 20; ucRetry++)
        {
            ...
            {
                // 100ms timeout, cancel the Down Reply MSG
                if(ucRetry == 19)
                {
                    // Disable Tx Read MSG
                    SET_REG_DP_TX_UPSTREAM_MSG_READ_DISABLE();
                ...
                {
                    // If HW Retry Times Reaches RETRY_NUM, reactive HW to read 1400h
                    if(ScalerGetBit(PBD_B1_AUX_IRQ_EVENT, _BIT1) == _BIT1)
                    {
                        ScalerSetByte(PBD_B1_AUX_IRQ_EVENT, _BIT1);

                        // Disable Tx Read MSG
                        SET_REG_DP_TX_UPSTREAM_MSG_READ_DISABLE();

                        // If DOWN_REP_MSG_RDY (2003h[4]) of downstream is still set, retry to read, otherwise cancel the down reply event.
                        if(ScalerDpTxAuxNativeRead(0x00, 0x20, 0x03, 1, pData) == _TRUE)
                        {
                            if((pData[0] & _BIT4) == 0x00)
                            {
                                SET_DP_TX_RD_BUFFER_STATUS(_DP_TX_READ_BUF_EMPTY);
